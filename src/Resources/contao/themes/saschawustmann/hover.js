var Theme = {
    ...
    setupProfileToggle: function () {
        ...
        Array.prototype.forEach.call(li, function (el) {
            var button = el.getFirst('span').getFirst('button'),
                menu = el.getFirst('ul'),
                elementClass = el.getFirst('span');
            if (!el || !button || !menu) return;

            if (elementClass.classList.contains('profile')) {
                button.setAttribute('aria-controls', 'tmenu__profile');
                button.setAttribute('aria-expanded', 'false');
                menu.id = 'tmenu__profile';
            } else if (elementClass.classList.contains('support')) {
                button.setAttribute('aria-controls', 'tmenu__support');
                button.setAttribute('aria-expanded', 'false');
                menu.id = 'tmenu__support';
            }
